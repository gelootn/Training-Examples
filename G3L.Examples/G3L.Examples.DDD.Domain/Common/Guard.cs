...
namespace G3L.Examples.DDD.Domain.Common
{
    public static class Guard
    {
        ...
        public static void AgainstDateBefore<TException>(DateTime start, DateTime mustBeAfter, string name = "Value")
            where TException : BaseDomainException, new()
        {
            if (start >= mustBeAfter)
            {
                return;
            }
            
            ThrowException<TException>($"{start} must be later then {mustBeAfter} ");
        }
